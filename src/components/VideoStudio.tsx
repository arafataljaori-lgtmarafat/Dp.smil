...
const TEMPLATES: Array<{
  ...
  {
    id: "split-slider",
    name: "مسطرة المقارنة (Split Slider)",
    nameEn: "Split Slider",
  ...
  {
    id: "spotlight-zoom",
    name: "العدسة الميكروسكوبية (Spotlight Zoom)",
    nameEn: "Spotlight Zoom",
  ...
  {
    id: "social-story-reel",
    name: "ريلز وتيك توك (Social Reel)",
    nameEn: "Social Story Reel",
    desc: "إيقاع سريع مع شريط تقدم علوي وطابع مخصص لقصص إنستغرام",
    badge: "Viral Reels",
    icon: "Smartphone",
  },
  {
  ...
  {
    id: "curtain-wipe",
    name: "المسح الكلاسيكي (Curtain Wipe)",
    nameEn: "Curtain Wipe",
    desc: "انتقال رصين مناسب لشاشات الانتظار في العيادة والموقع الإلكتروني",
    badge: "كلاسيك",
    icon: "Monitor",
  },
  {
    id: "pulse-reveal",
    name: "نبض الابتسامة (Pulse Reveal)",
    nameEn: "Pulse Reveal",
    desc: "تحول ديناميكي مع نبضات بصرية توضح الفرق الجذري في اللون والإطباق",
    badge: "ديناميكي",
    icon: "Repeat",
  },
];
...
export const VideoStudio: React.FC<VideoStudioProps> = ({
  ...
  const drawFrame = useCallback(
    ...
    [config, loadedImages, beforePhoto?.alignment, afterPhoto?.alignment]
  );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      {/* Top Studio Bar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/80 backdrop-blur-md p-4 rounded-2xl border border-slate-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs px-2 py-0.5 rounded-md bg-teal-500/10 text-teal-400 border border-teal-500/20 font-mono">
                {patientCase.patientCode}
              </span>
            </div>
            <p className="text-xs text-slate-400">{patientCase.treatmentType}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 bg-slate-950 p-1 rounded-xl border border-slate-800">
          {(["9:16", "4:5", "1:1", "16:9"] as AspectRatioType[]).map((ar) => (
            ...
            >
              {ar === "9:16" && <Smartphone className="w-3.5 h-3.5" />}
              {ar === "4:5" && <Square className="w-3.5 h-3.5" />}
              {ar === "1:1" && <Grid className="w-3.5 h-3.5" />}
              {ar === "16:9" && <Monitor className="w-3.5 h-3.5" />}
              <span>{ar}</span>
            </button>
          ))}
        </div>
      ...
      {/* Main Studio Grid: Video Player on Left, Control Tabs on Right */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* Left Side: Video Canvas Player & Scrub Controls (7 cols) */}
        <div className="lg:col-span-7 bg-slate-900 border border-slate-800 rounded-2xl p-4 sm:p-5 flex flex-col items-center space-y-4 shadow-xl">
          {/* Canvas Wrapper */}
          <div
            className="relative bg-slate-950 rounded-xl overflow-hidden shadow-2xl border border-slate-800/80 flex items-center justify-center max-h-[560px] w-full"
            style={{
          ...
          >
            ...
            {/* Quick Badge overlay in preview */}
            <div className="absolute top-3 left-3 bg-slate-950/80 backdrop-blur-md px-2.5 py-1 rounded-lg border border-slate-800 text-[11px] text-teal-400 font-mono flex items-center gap-1.5 pointer-events-none">
          ...
          <div className="w-full space-y-2 bg-slate-950/80 p-3.5 rounded-xl border border-slate-800">
            ...
            <div className="relative w-full">
              ...
              <div
                className="absolute top-0 bottom-0 pointer-events-none bg-teal-500/20 rounded border-x border-teal-400/40"
                style={{
            ...
            <div className="flex items-center justify-between gap-3 text-xs pt-1">
              <div className="flex items-center gap-2">
                ...
                <button
                  id="btn-toggle-play"
                  onClick={togglePlay}
                  className="p-2 rounded-lg bg-teal-500 hover:bg-teal-400 text-slate-950 transition-colors font-bold shadow-md shadow-teal-500/20"
                >
                  {isPlaying ? (
                ...
                {/* Time Display */}
                <span className="font-mono text-slate-300 font-bold px-2">
                  {currentTime.toFixed(2)}s / {config.duration.toFixed(1)}s
                </span>
              </div>

              {/* Center Keyframe labels */}
              <div className="hidden sm:flex items-center gap-2 text-[10px] text-slate-400">
                <span className="px-1.5 py-0.5 rounded bg-rose-950/70 text-rose-300 border border-rose-500/30">
                  Before
                </span>
                <span>➔</span>
                <span className="px-1.5 py-0.5 rounded bg-teal-950/70 text-teal-300 border border-teal-500/30">
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="bg-slate-900 border border-slate-800 text-slate-300 rounded-lg px-2 py-1 text-[11px] font-mono focus:border-teal-500"
                >
                ...
                <button
                  ...
                  title="التقاط صورة ثابتة للإطار الحالي"
                >
        ...
        {/* Right Side: Studio Controls & Configuration Tabs (5 cols) */}
        <div className="lg:col-span-5 bg-slate-900 border border-slate-800 rounded-2xl flex flex-col shadow-xl overflow-hidden min-h-[580px]">
          {/* Tab Navigation */}
          ...
          {/* Tab Content Body */}
          <div className="p-5 overflow-y-auto max-h-[620px] flex-1 space-y-4">
            {/* 1. TEMPLATES TAB */}
            {activeTab === "template" && (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-300">
                    اختر قالب الفيديو المناسب لحالتك السريرية:
                  </span>
                  <button
                  ...
                  {TEMPLATES.map((tmpl) => {
                    ...
                    return (
                      <div
                        ...
                        onClick={() => updateConfig({ ...config, templateId: tmpl.id }, `تغيير القالب إلى ${tmpl.name}`)}
                      ...
                      >
                        <div className="flex-1 space-y-1">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] px-1.5 py-0.2 rounded-full bg-teal-500/10 text-teal-400 border border-teal-500/20 font-medium">
                              {tmpl.badge}
                            </span>
                          </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      ...
                      <button
                        ...
                        className={`p-2 rounded-lg text-xs font-medium text-right border transition-all ${
                  ...
                  {config.zoomEffect !== "static" && (
                    <div className="pt-2 space-y-1">
                      <div className="flex justify-between text-[11px] text-slate-400">
                        <span>شدة التكبير (Intensity)</span>
              ...
              <div className="space-y-4">
                ...
                {config.textOverlays.length === 0 ? (
                  <div className="p-6 text-center text-xs text-slate-500 bg-slate-950/40 rounded-xl border border-slate-800/60">
                    لا توجد نصوص إضافية. انقر على "إضافة نص" لكتابة مميزات الحالة السريرية.
                  </div>
                ) : (
                  <div className="space-y-3">
