...
export type DurableEditorCheckpoint = EditorPersistenceState<any> & { readonly phase: 'saved' | 'clean' };

/** Serializes a binding mutation after the latest local document reaches a durable CAS checkpoint. */
export async function runWithDurableEditorCheckpoint<T>(flush: () => Promise<EditorPersistenceState<any> | null>, mutation: (checkpoint: DurableEditorCheckpoint) => Promise<T>): Promise<T> {
